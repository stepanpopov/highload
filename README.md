...
Будем использовать Per-Flow, чтобы пакеты с одного IP попадали на один и тот же балансировщик. А также использовать Resilient/Consistent Hashing для минимизации разрыва TCP при добавлении новых путей.

Каждый ECMP маршрут будет связан с репликасетом из L7 балансировщиков, резервируемых с помощью keepalived VRRP технологий.
Keepalived VRRP будет осуществлять отказоустойчивость.
