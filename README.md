...
<img src="https://github.com/stepanpopov/highload-google-drive/assets/77172612/e7dbcc7a-caa0-44d7-8572-ffecf3291935" width="600">
...
- file_uuid и chunk_id - составной btree index

Шардирование для таблицы Files по file_uuid
Шардирование для таблицы Chunks по file_uuid и chunk_id
...
Рассмотрим механизм обновления чанков файлов:
Когда пользователь впервые загружает файл, он разбивается на чанки, чанки жмутся и записываются в S3, пользователю приходит информация о чанках.
Эта информация отправляется на все синхронизируемые устройства, которые держут long polling коннект.

На клиенте у пользователя постоянно работает процесс, который проверяет изменения файлов на диске и в случае изменения какого либо чанка, отправляет номер чанка и новые данные. 
Бэкэнд сервис обновляет информацию в базе данных и загружает новые данные в S3. 
Затем бэкенд кладет информацию с номером чанка и ссылкой на s3 в очеред. 
Синхронизируемым устройствам, которые держат long polling коннект, приходит информация, о новых чанках.

При новой попытке синхронизации с облачным сервисом, клиент отправляет информацию о чанках и их версиях, и если в базе данных есть более новые версии, клиент узнает об этом и скачивает обновленную информацию.

### Технологии
- Rust for Web Assembly Envoy module
- Golang
- Envoy
- React + TS
...
### Архитектура
![image](https://github.com/stepanpopov/highload-google-drive/assets/77172612/6d87e409-ec35-4dc8-9647-05fa9540f275)
