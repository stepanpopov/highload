...
Также Tarantool будет использоваться в качестве engine для реализации pub/sub архитектуры в Centryfugo.
Расчет для Centryfugo Engine:
- запись события будет занимать (16 байт uuid юзера + ~90 байт путь до файла/директории + 4 байта на информацию о действии) * 720 pub/секунду * 1 час ttl = 271 GB
...
Предположим у всех пользователей включеная автоматическая синхронизация, DAU = 5.5 M.
Расчет нагрузки для балансировшиков:
Nginx = RPS фронтовых микросервисов = 2490 + 684 + 126 + 522 + 120 + 126 = 4068 RPS.
При расчете учтем, что nginx выполняет ssl терминацию.

Envoy = RPS фронтовых микросервисов + авторизация = 4068 * 2 = 8136 RPS.

Сервисы написаны на Golang, будем считать, что каждый запрос стоит 
