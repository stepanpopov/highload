...
Общее: 
- 90 байт * 700 m = 58 GiB\

### Схема таблиц для файлов каждого юзера:
<img src="https://github.com/stepanpopov/highload-google-drive/assets/77172612/a7291d4f-3d57-49f0-a71e-84feaa92ae75" width="300">
...
### Схема таблиц для хранения файлов небольшого размера каждого юзера:
<img src="https://github.com/stepanpopov/highload-google-drive/assets/77172612/a0f2ff2f-2dc5-4496-87cc-054c1333d0cc" width="300">
...
Как только общий размера файлов небольшого размера у юзера достигает 1 GB, делаем bulk инсерт в s3 с помощью AWS Snowball.
...
Общие права будут истекать через 2 недели.\
...
Общее: 
- (16 байт + 16 байт + 1 байт) * 67M(MAU) * 3200(среднее кол-во файлов у пользователя) * 3 / 10 = 972 GiB.

## Физическая схема данных
Таблицы USER_FILES и USER_FILES_SMALL будем хранить в NoSql disk db, которая умеет делать шардирование из коробки. (например Cassandra)

Данные файлов будем хранить в облачном S3 хранилище и небольшие файлы блобами в NoSql бд.
Переливку небольших файлов батчами в S3 будем делать через AWS Snowball.
...
Для удаления протухших записей в ACL в tarantool есть модуль TTL для настройки удаления старых записей.
