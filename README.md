...
На клиенте у пользователя постоянно работает процесс, который проверяет изменения файлов на диске и локально сохраняет дерево файлов с их версиями.
...
При загрузке нового файла, сервис Upload асинхронно отправляет сервису Notifier событие об изменении, которое впоследствии отправляется по веб сокет соединению.
### Технологии
- Rust for Web Assembly Envoy module для поддержки логгирования и запросов на авторизацию
- Golang для микросервисов
- Envoy -bлокальный балансировщик
- React + TS фронтенд
...
### Архитектура
![image](https://github.com/stepanpopov/highload-google-drive/assets/77172612/35f52456-2ee3-4789-9644-4e9b2ad0e136)
